...
const L = {
  ...
  h1: { fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:800, fontSize:'clamp(38px,12vw,76px)', lineHeight:0.96, letterSpacing:'clamp(-2.4px,-0.5vw,-1px)', color:'#F5F0E8', margin:'18px 0 0' },
  sub: { fontSize:'clamp(15px,4vw,19px)', color:'rgba(245,240,232,0.68)', maxWidth:560, lineHeight:1.7, margin:'18px 0 0' },
  ...
  cIcon: { fontSize:'clamp(22px,7vw,30px)' },
  cTitle: { fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:800, fontSize:'clamp(14px,3.8vw,16px)', color:'#F5F0E8' },
  cSub: { fontSize:'clamp(13px,3.5vw,14px)', color:'rgba(245,240,232,0.58)', lineHeight:1.6 },
  ...
  inp: { width:'100%', padding:'clamp(10px,3vw,12px) 13px', border:'1.5px solid rgba(255,255,255,0.12)', borderRadius:10, fontSize:'clamp(14px,4vw,16px)', background:'#0D0B08', color:'#F5F0E8', outline:'none', fontFamily:"'DM Sans',sans-serif" },
...
export default function Landing() {
  ...
  return (
    <div style={L.page}>
      ...
      <section style={L.hero}>
        ...
        <div style={{ background:'radial-gradient(circle at top right,rgba(232,68,10,0.28),transparent 45%),linear-gradient(180deg,rgba(255,255,255,0.09),rgba(255,255,255,0.035))', border:'1px solid rgba(255,255,255,0.12)', borderRadius:28, padding:'clamp(18px,5vw,28px)', boxShadow:'0 28px 80px rgba(0,0,0,0.35)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
            <div style={{ fontFamily:"'Plus Jakarta Sans'", fontWeight:800, fontSize:'clamp(18px,5vw,24px)' }}>Live counter</div>
          ...
          {['Dine-in table T4 · ₹1,246', 'Takeaway order · ₹498', 'Kitchen: 3 items preparing'].map((row, idx) => (
            <div key={row} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 0', borderTop: idx ? '1px solid rgba(255,255,255,0.08)' : 'none' }}>
              ...
              <span style={{ flex:1, color:'rgba(245,240,232,0.78)', fontSize:'clamp(13px,3.6vw,15px)' }}>{row}</span>
      ...
      <section style={L.section}>
        <h2 style={{ fontFamily:"'Plus Jakarta Sans'", fontSize:'clamp(26px,7vw,42px)', letterSpacing:'-1px', marginBottom:18 }}>FAQs</h2>
        <div style={{ display:'grid', gap:12 }}>{FAQS.map(f => <details key={f.q} style={{ ...L.card, padding:'16px 18px' }}><summary style={{ cursor:'pointer', fontWeight:800, fontSize:'clamp(14px,4vw,16px)' }}>{f.q}</summary><p style={{ margin:'10px 0 0', color:'rgba(245,240,232,0.6)', lineHeight:1.6, fontSize:'clamp(13px,3.7vw,15px)' }}>{f.a}</p></details>)}</div>
      </section>
