  ...
  return (
    <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden', marginBottom:8 }}>
      <div style={{ padding:'8px 10px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between',
        ...
        <div>
          <div style={{ fontWeight:800, fontSize: 'var(--fs-13)' }}>#{order.order_number} · Table {order.table_number}</div>
          <div style={{ fontSize: 'var(--fs-10)', color:ageColor, fontWeight:600 }}>{age}m ago</div>
        </div>
        {order.status==='pending' && <button onClick={() => onStatusChange(order.id,'preparing')} style={{ background:'#3B82F6', color:'#fff', border:'none', borderRadius:6, padding:'5px 10px', fontSize: 'var(--fs-11)', fontWeight:700, cursor:'pointer' }}>🔥 Start</button>}
        {order.status==='preparing' && <button onClick={() => onStatusChange(order.id,'ready')} style={{ background:'#22C55E', color:'#fff', border:'none', borderRadius:6, padding:'5px 10px', fontSize: 'var(--fs-11)', fontWeight:700, cursor:'pointer' }}>✅ Ready</button>}
        {order.status==='ready' && <span style={{ fontSize: 'var(--fs-11)', color:'#22C55E', fontWeight:700 }}>Waiting for waiter</span>}
      </div>
      <div style={{ padding:'6px 10px' }}>
        {activeItems.map(item => (
          <div key={item.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 0', fontSize: 'var(--fs-12)', borderBottom:'1px solid var(--border)' }}>
            ...
            <button onClick={() => rejectItemInOrder(item, order.id)}
              style={{ fontSize: 'var(--fs-10)', padding:'1px 7px', background:'rgba(239,68,68,0.08)',
        ...
        {activeItems.filter(i => i.notes).map(i => (
          <div key={i.id+'n'} style={{ fontSize: 'var(--fs-11)', color:'var(--amber)', padding:'2px 0' }}>📝 {i.product_name}: {i.notes}</div>
        ))}
        {rejectedItems.length > 0 && <div style={{ fontSize: 'var(--fs-11)', color:'var(--red)', marginTop:4 }}>⚠ {rejectedItems.length} unavailable</div>}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden', marginBottom:8 }}>
      <div style={{ padding:'8px 12px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:8, background:'var(--card2)' }}>
        <span style={{ fontSize: 'var(--fs-20)' }}>{icon||'🍽'}</span>
        <span style={{ fontWeight:700, fontSize: 'var(--fs-14)' }}>{itemName}</span>
        <span style={{ marginLeft:'auto', background:'var(--brand)', color:'#fff', fontSize: 'var(--fs-11)', fontWeight:800, padding:'2px 8px', borderRadius:10 }}>
      ...
      {entries.map(e => (
        <div key={e.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 12px', borderBottom:'1px solid var(--border)',
          ...
          <span style={{ fontSize: 'var(--fs-10)', fontWeight:700, padding:'2px 6px', borderRadius:6, background:`${tableColor(e.orderStatus)}20`, color:tableColor(e.orderStatus) }}>
            T{e.tableLabel}
          </span>
          <span style={{ fontSize: 'var(--fs-12)', color:'var(--text2)' }}>×{e.qty}</span>
          {e.notes && <span style={{ fontSize: 'var(--fs-11)', color:'var(--amber)', flex:1 }}>📝 {e.notes}</span>}
          {e.status==='rejected'
            ? <button onClick={() => onRestore(e)} style={{ fontSize: 'var(--fs-10)', padding:'2px 8px', background:'var(--green-bg)', color:'var(--green)', border:'1px solid rgba(34,197,94,0.3)', borderRadius:6, cursor:'pointer' }}>Restore</button>
            : <button onClick={() => onReject(e)} style={{ fontSize: 'var(--fs-10)', padding:'2px 8px', background:'rgba(239,68,68,0.08)', color:'var(--red)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:6, cursor:'pointer', marginLeft:'auto' }}>Unavail</button>
          }
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
        <h2 style={{ fontFamily:"'Plus Jakarta Sans'", fontWeight:800, fontSize: 'var(--fs-16)', flex:1 }}>🍳 Kitchen Display</h2>
        <button onClick={load} style={{ background:'none', border:'1px solid var(--border)', borderRadius:6, padding:'4px 10px', fontSize: 'var(--fs-12)', color:'var(--text2)', cursor:'pointer' }}>↻</button>
        <div style={{ display:'flex', background:'var(--card2)', borderRadius:8, padding:2, gap:2 }}>
          {[['kanban','By Order'],['items','By Item']].map(([v,l]) => (
            <button key={v} onClick={() => setView(v)} style={{ background:view===v?'var(--card)':'transparent', border:'none', borderRadius:6, padding:'4px 10px', fontSize: 'var(--fs-12)', fontWeight:view===v?600:400, color:view===v?'var(--text)':'var(--text2)', cursor:'pointer' }}>{l}</button>
          ))}
        </div>
      </div>
      ...
      : view==='kanban' ? (
          ...
          {cols.map(col => (
            <div key={col.label}>
              <div style={{ padding:'6px 10px', borderRadius:8, background:col.bg, border:`1px solid ${col.color}30`, marginBottom:8, display:'flex', justifyContent:'space-between' }}>
                <span style={{ fontSize: 'var(--fs-12)', fontWeight:700, color:col.color }}>{col.label}</span>
                <span style={{ fontSize: 'var(--fs-11)', color:col.color, fontWeight:600 }}>{col.items.length}</span>
              </div>
              {col.items.length===0
                ? <div style={{ textAlign:'center', padding:20, color:'var(--text3)', fontSize: 'var(--fs-12)' }}>Empty</div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:12 }}>
          {Object.keys(itemGroups).length===0
            ? <div style={{ textAlign:'center', padding:40, color:'var(--text2)' }}><div style={{ fontSize: 'var(--fs-32)', marginBottom:8 }}>✅</div><div>All clear</div></div>
