...
function OrderTypeBar() {
  ...
  const tables = Array.from({ length: tableCount }, (_, i) => ({
    num: i + 1,
    name: settings?.table_names?.[i] || `T${i + 1}`,
  }))
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0, flexWrap: 'wrap' }}>
      ...
      {/* Table button — only when Dine In */}
      {orderType === 'dine' && (
        <>
          ...
          {tablePickerOpen && (
            <div style={{
              ...
              <div onClick={e => e.stopPropagation()} style={{
                ...
                boxShadow: '0 12px 40px rgba(0,0,0,0.5)', zIndex: 400,
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {tables.map(t => (
                    <button key={t.num} onClick={() => { setTable(t.num, t.name); setTablePickerOpen(false) }}
                      style={{
...
function MenuPane() {
  ...
  const filtered = useMemo(() =>
    activeCat === 'all' ? products : products.filter(p => String(p.category_id) === activeCat),
    [products, activeCat]
  )

  // Group filtered products by category
  const groups = useMemo(() => {
    if (activeCat !== 'all') return [{ name: '', items: filtered }]
    const map = {}
    filtered.forEach(p => {
              ...
              {group.items.map(p => {
                ...
                return (
                  <button key={p.id}
                    ...
                    disabled={oos}
                    style={{
                      background: inCart ? 'var(--brand-lt2)' : 'var(--card)',
                      border: `1.5px solid ${inCart ? 'rgba(232,68,10,0.25)' : 'var(--border)'}`,
                    ...
                    {oos && (
                      <span style={{ fontSize: 10, color: 'var(--red)', fontWeight: 600 }}>Out of stock</span>
                    )}
                  </button>
                )
              })}
...
function CartPane({ onCheckout }) {
  ...
  const typeLabel =
    orderType === 'dine' ? `🍽 Table ${tableName || tableNumber || '?'}`
          ...
          <div key={item.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
              <button onClick={() => removeFromCart(item.id)} style={{
                ...
                color: 'var(--text)', fontSize: 14, lineHeight: 1,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                cursor: 'pointer',
              }}>−</button>
      ...
      {items.length > 0 && (
        <div style={{ borderTop: '1px solid var(--border)', padding: 14, flexShrink: 0 }}>
          ...
          <button onClick={onCheckout} style={{
            ...
            border: 'none', borderRadius: 'var(--r)', padding: 13,
...
function CheckoutModal({ open, onClose, onSuccess }) {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      // 3. If UPI — update status after confirmation, otherwise mark paid
      if (payMethod !== 'upi') {
      ...
      onSuccess({ ...order, total, payMethod, items, upiId })
      handleClose()
    } catch (err) {
  ...
  const orderLabel =
    orderType === 'dine' ? `🍽 Dine In — Table ${tableName || tableNumber || '?'}`
    : orderType === 'takeaway' ? '🛍 Takeaway'
    : '🏪 Stall'
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Checkout"
      footer={
        step === 1 ? (
          <button form="step1form" type="submit" style={{
            width: '100%', background: 'var(--brand)', color: '#fff',
            border: 'none', borderRadius: 8, padding: 13, fontWeight: 700, fontSize: 14,
          }}>
            Next: Payment →
          </button>
        ) : (
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setStep(1)} style={{
              flex: '0 0 80px', background: 'var(--card2)', color: 'var(--text)',
              border: '1px solid var(--border)', borderRadius: 8, padding: 12, fontWeight: 600,
            }}>← Back</button>
            <button onClick={placeOrder} disabled={loading} style={{
              flex: 1, background: 'var(--brand)', color: '#fff',
              border: 'none', borderRadius: 8, padding: 12, fontWeight: 700, fontSize: 14,
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            }}>
      ...
      <div style={{ background: 'var(--bg)', borderRadius: 8, padding: '10px 12px', marginBottom: 16, fontSize: 12 }}>
        <div style={{ color: 'var(--brand)', fontWeight: 700, marginBottom: 6 }}>
          {orderLabel} · {items.length} item{items.length !== 1 ? 's' : ''}
        </div>
        {items.map(i => (
      ...
      {/* STEP 1 */}
      {step === 1 && (
        <form id="step1form" onSubmit={nextStep} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Field label="Email (optional)">
            <input value={email} onChange={e => setEmail(e.target.value)}
              type="email" placeholder="email@example.com" style={inputStyle} />
          </Field>
        </form>
      )}
            ...
            {PAY_METHODS.map(m => {
              ...
              return (
                <button key={m.id} onClick={() => setPayMethod(m.id)}
                  ...
                  <div style={{ fontSize: 10, color: warn ? 'var(--amber)' : 'var(--text3)', marginTop: 2 }}>
                    {warn ? '⚠ UPI not set' : m.sub}
                  </div>
                </button>
              )
            })}
...
function Field({ label, required, children }) {
  return (
    <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text2)', textTransform: 'uppercase', letterSpacing: '0.4px' }}>
        {label}
      </span>
      {children}
    </label>
  ...
  return (
    <Modal open={!!order} onClose={onClose} title="Order Placed ✓"
      ...
      <div style={{ textAlign: 'center', padding: '8px 0' }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--brand)', margin: '8px 0 4px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
          {order.customer_phone && (
            <a href={`https://wa.me/${order.customer_phone.replace(/\D/g, '')}?text=Your%20bill%20from%20BITE.%20%E2%82%B9${order.total}`}
              target="_blank" rel="noreferrer"
              style={{ ...shareBtn, background: '#25D366', borderColor: 'transparent' }}>
              💬 WhatsApp
            </a>
          )}
          <a href={`/receipt.html?id=${order.id}`} target="_blank" rel="noreferrer"
            style={{ ...shareBtn }}>
            🧾 Receipt
          </a>
        </div>
...
const shareBtn = {
  ...
  background: 'var(--card2)', border: '1px solid var(--border2)',
  borderRadius: 8, color: 'var(--text)', fontSize: 12, fontWeight: 600,
...
export default function OrderPage() {
  ...
  const { orderType, tableNumber } = useStore()
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', flexDirection: 'column' }}>
      ...
      <CheckoutModal
        ...
        onSuccess={order => setSuccessOrder(order)}
      />
      <SuccessModal
        order={successOrder}
